...
export default class Binance {
  ...
  async start() {
    try {
      ...
      let ethOnlyPairs = await Promise.all(
        pairs.filter(pair => pair.includes("eth"))
      );
      logger.info(ethOnlyPairs);
      let streams = "";
      //join pairs to streaming url
      await Promise.all(
        ethOnlyPairs.map((pair, i) => {
    ...
    if (u > state.lastUpdateId) {
      if (
        ...
        if (b.length >= 1) {
          ...
          logger.info(`UPDATE BUY - ${stream}-${u}-${JSON.stringify(order)}`);
        ...
        if (a.length >= 1) {
          ...
          let order = Object.assign({}, ..._orders);
          logger.info(`UPDATE SELL - ${stream}-${u}-${JSON.stringify(order)}`);
