...
const { readableBalance, parseAmount, parseEth} = require('../../../utils/formatter');
...
import { FROM, FACTORY_ADDRESS, INFURA_NODE, 
  NODE1, NODE2, NODE3, NODE4, NODE5, NODE6, NODE7, NODE8 } from 'babel-dotenv';
...
export default class Uniswap {
  ...
  async getEthToTokenOutputPrice(tokenAddress, tokenBought) {
    try {
        ...
        console.log(`ETH Sold for ${tokenBought} Tokens = ${readableBalance(ethSold, this._getTokenDecimal(tokenAddress))}`);
        const {minimum , maximum} = this._calculateSlippageBounds(ethSold, false);
        console.log(`Max ETH Sold for ${tokenBought} Tokens = ${readableBalance(maximum, this._getTokenDecimal(tokenAddress))}`);
        return {
            tokenBought 
        }
    }
  ...
  async getTokenToEthOutputPrice(tokenAddress, ethBought) {
    try {
        ...
        //console.log(`Tokens sold for ${ethBought} ETH = ${readableBalance(tokenSold, this._getTokenDecimal(tokenAddress))}`);
  ...
  async getTokenToTokenInputPrice(inputTokenAddress, outputTokenAddress, inputTokenAmount){
    ...
    let inputEth = readableBalance(ethReceived, this._getTokenDecimal(inputTokenAddress));
    let outputTokens = await this.getEthToTokenInputPrice(outputTokenAddress, inputEth);
    console.log(`Tokens - ${outputTokens}`)
    return readableBalance(outputTokens, this._getTokenDecimal(outputTokenAddress));
  }
  ...
  async getExchange(tokenAddress) {
    try{
        //instantiate factory contract
      const factoryContract = new _web3.eth.Contract(factoryABI, FACTORY_ADDRESS);
  ...
  async getExchangeForListings(tokenAddress, nodeCount = 0) {
    ...
    const factoryContract = new web3.eth.Contract(factoryABI, FACTORY_ADDRESS);
    let exchangeAddress = await factoryContract.methods.getExchange(tokenAddress).call();
  ...
  async getRate(from, to, amount, fromAddress = null, toAddress = null){
    if(to == 'ETH'){
      ...
      return await this.getTokenToTokenInputPrice(fromAddress, toAddress, amount);
    }
  }
