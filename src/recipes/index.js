import { EXCHANGEC, EXCHANGED, BOOK_PERCENT, TIMER, BASE, TOKEN } from 'babel-dotenv';
...
async function initializeBuyCSellD(token){
    ...
    if(prcnt * 100 < BOOK_PERCENT){
        console.log(`Percent - ${prcnt * 100} No Fun....Ditching ${token}-${BASE}`);
    }
    else {
        let message = {
            buyFrom: EXCHANGEC,
...
async function initializeSellCBuyD(token){
    ...
    console.log(`--------- BUY (${EXCHANGED}) ------ SELL(${EXCHANGEC})-------------`);
    let exc = new ExchangeC(EXCHANGEC);
    let exd = new ExchangeD(EXCHANGED);
    let pair = exc.formatToPair(token, BASE);
    console.log(`Watching ${token}-${BASE}`);
    ...
    let rates = await exd.getRate(BASE, token, (buy[1][1]).toString(), null, tokenAddress);
    console.log(rates);
    let prcnt = 0; 
    if(rates > 0){
