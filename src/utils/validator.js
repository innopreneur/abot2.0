...
function findBetterDeal(priceUniswap, tickerBittrex) {
  ...
  if (_calculatePercentageProfit(priceUniswap, Bid) >= BOOK_PERCENT) {
    ...
    //Buy on Uniswap
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex(${Bid})| >= ${BOOK_PERCENT}%`
    );
    logger.info("BUY on Uniswap ------ SELL on Bittrex");
    ...
    //Buy on Bittrex
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex(${Ask})| >= ${BOOK_PERCENT}%`
    );
    logger.info("SELL on Uniswap ------ BUY on Bittrex");
  } else {
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex SELL (${Bid})| = ${_calculatePercentageProfit(
        priceUniswap,
        Bid
      )} <= ${BOOK_PERCENT}%`
    );
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex BUY (${Ask})| = ${_calculatePercentageProfit(
        Ask,
        priceUniswap
      )} <= ${BOOK_PERCENT}%`
    );
...
function calculateSellBuyProfitability(priceUniswap, priceBittrex) {
  ...
  if (_calculatePercentageProfit(_bittrex, priceUniswap) >= BOOK_PERCENT) {
    ...
    //Buy on Bittrex
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex(${_bittrex})| >= ${BOOK_PERCENT}%`
    );
    logger.info("SELL on Uniswap ------ BUY on Bittrex");
    ...
    logger.info(
      `| Uniswap SELL (${priceUniswap}) - Bittrex BUY (${_bittrex})| = ${_calculatePercentageProfit(
        _bittrex,
        priceUniswap
      )} <= ${BOOK_PERCENT}%`
    );
...
function calculateBuySellProfitability(priceUniswap, priceBittrex) {
  ...
  if (_calculatePercentageProfit(priceUniswap, _bittrex) >= BOOK_PERCENT) {
    ...
    //Buy on Uniswap
    logger.info(
      `| Bittrex SELL (${_bittrex}) - Uniswap BUY (${priceUniswap}) | >= ${BOOK_PERCENT}%`
    );
    logger.info("BUY on Uniswap ------ SELL on Bittrex");
    ...
    logger.info(
      `| Uniswap(${priceUniswap}) - Bittrex SELL (${_bittrex})| = ${_calculatePercentageProfit(
        priceUniswap,
        _bittrex
      )} <= ${BOOK_PERCENT}%`
    );
