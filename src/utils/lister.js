...
import { connect, disconnect, updateListings, getListings, updatePairs} from './storage';
...
async function updateUniswapListings(){
        ...
        let exchanges = [];
        let count = 0;
        let tokens = await Promise.all(data.filter((token) => {
            return (token.platform && token.platform.name == 'Ethereum')
        }));

        await Promise.all(tokens.map(async (listing, index) => {
                try { 
                    if(index > 0 && index % 200 == 0){
                        console.log(`Going to wait for counts ${index}`)
                        wait(30);
                    }
                    // below (index % 8) defines which node to use. We use round robin method
                    console.log(`${index} : ${listing.symbol}`)
                    let exchangeAddress = await uniswap.getExchangeForListings(listing.platform.token_address, index % 8);
                    console.log(`${index} : ${listing.symbol} : ${exchangeAddress}`)
                    if(exchangeAddress != '0x0000000000000000000000000000000000000000'){
                        ...
                        console.log(`Index ${index} :: Node ${count % 8} :: ${listing.symbol},${listing.name},${listing.platform.token_address},${listing.quote.USD.market_cap}, ${exchangeAddress}`);
...
async function getListing(listing) {
    try {
        let uniswap = new Uniswap(store);
        let exchangeAddress = await uniswap.getExchangeForListings(listing.platform.token_address, 0);
        ...
        if(exchangeAddress != '0x0000000000000000000000000000000000000000'){
            ...
            mcap = listing.quote.USD.market_cap,
            exchange = exchangeAddress;
            let {eth: ethReserve, token: tokenReserve} = await uniswap.getPoolSize(listing.symbol, listing.platform.token_address);
            let pricePerToken = await uniswap.getTokenToEthInputPrice(listing.platform.token_address, 1);
            console.log(`${listing.symbol},${listing.name},${listing.platform.token_address},${listing.quote.USD.market_cap}, ${exchangeAddress}, ${ethReserve}, ${tokenReserve}, ${pricePerToken}`);
...
async function _getUniswapExchange(uniswap, listing){
    ...
    let exchangeAddress = await uniswap.getExchangeForListings(listing.platform.token_address);
    if(exchangeAddress != '0x0000000000000000000000000000000000000000'){
        let symbol = listing.symbol, 
        name = listing.name, 
        address = listing.platform.token_address,
        mcap = listing.quote.USD.market_cap,
        exchange = exchangeAddress;
        console.log(`${listing.symbol},${listing.name},${listing.platform.token_address},${listing.quote.USD.market_cap}, ${exchangeAddress}`);
...
async function matchDexagBittrex(){
    ...
    await Promise.all(dListings.map(async (dItem) => {
            if(dItem){
               let market = await Promise.all(bListings.filter(bItem => {
